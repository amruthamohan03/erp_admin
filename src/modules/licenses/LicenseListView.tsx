...
/**
 * One licence list, two directions.
 *
 * Import and Export licences are the SAME records — `license_t` is direction-
 * agnostic and `kind_id` is the only discriminator (§4.1). So the two screens
 * are this component with a different scope, not two 460-line copies that would
 * need every later fix applying twice (§4.10).
 */
export interface LicenseListConfig {
  /** Which side of the business this screen shows. */
  useFor: 'import' | 'export';
  /** The <h1>. */
  heading: string;
  /** The table's own title. */
  listTitle: string;
  /** Route prefix for create/edit links, e.g. '/export-licenses'. */
  basePath: string;
  /** The toolbar button's words — 'New Export License' (§4.35). */
  createLabel: string;
  /** The page-config slug the view/edit form runs on (§4.12). */
  slug: string;
}
...
function buildParams(f: FilterState, useFor: string): URLSearchParams {
  const p = new URLSearchParams();
  // Every request this screen makes carries its direction, so the grid, the
  // cards and the Excel export can never report on different books.
  p.set('use_for', useFor);
...
export default function LicenseListView({ config }: { config: LicenseListConfig }) {
  ...
  function exportAll(): void {
    const params = buildParams(applied, config.useFor);
  ...
  }, [applied, activeCard, page, pageSize, search, config.useFor]);
    ...
    (async () => {
      try {
        const [cardsRes, statsRes] = await Promise.all([
          ...
          fetch(`/api/v1/licenses/stats?use_for=${config.useFor}`).then((r) => r.json()),
        ]);
        if (cancelled) return;
        if (cardsRes.ok) {
          setCards(
  ...
  // Filter dropdown options — refetched when the direction changes, because the
  // Kind list is scoped to it.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [c, t, k, b] = await Promise.all([
        ...
        fetchOptions(`kinds?group=${config.useFor}`, 'kind_name'),
    ...
    <>
      <div className="card p-4 mb-4">
        <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
          <FileText className="h-5 w-5 text-primary-600" /> {config.heading}
        </h1>
      </div>
      ...
      <DataTable<LicenseRow>
        ...
        toolbar={
          ...
          <Link href={`${config.basePath}/new`} className="btn-primary btn-sm">
      ...
      {viewId !== null && (
        <RecordViewModal
          slug={config.slug}
          entityId={viewId}
          editHref={`${config.basePath}/${viewId}`}
