...
/**
 * One licence dashboard, two directions (§4.10).
 *
 * The Import and Export Licence dashboards are the same aggregates over a
 * different book — `use_for` scopes every figure server-side — so this is one
 * component with a config rather than two copies of 320 lines.
 */
export interface LicenseDashboardConfig {
  useFor: 'import' | 'export';
  heading: string;
  /** Route prefix for the list and row links, e.g. '/export-licenses'. */
  basePath: string;
  /** The words on the button back to the list. */
  listLabel: string;
}

export default function LicenseDashboardView({ config }: { config: LicenseDashboardConfig }) {
  const [d, setD] = useState<Dash | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetch(`/api/v1/licenses/dashboard?use_for=${config.useFor}`)
      .then((r) => r.json())
      .then((j) => {
        if (cancelled) return;
    ...
    <>
      <div className="card mb-4 flex flex-wrap items-center justify-between gap-3 p-4">
        <h1 className="flex items-center gap-2 text-xl font-bold text-foreground">
          <TrendingUp className="h-5 w-5 text-primary-600" /> {config.heading}
        </h1>
        <Link href={config.basePath} className="btn-neutral btn-sm">
          <FileCheck className="h-4 w-4" /> {config.listLabel}
        </Link>
      </div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-7">
        {TILES.map((t) => {
          ...
          return (
            <Link
              key={t.key}
              href={t.card === 'all' ? config.basePath : `${config.basePath}?card=${t.card}`}
      ...
      <div className="card mt-4 p-4">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <Link href={`${config.basePath}?card=expiring`} className="btn-neutral btn-sm">
            See all expiring
          </Link>
        </div>
              ...
              <tbody>
                {(d?.expiring_soon ?? []).map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="text-right">
                      <Link href={`${config.basePath}/${r.id}`} className="btn-edit btn-icon" title="Open">
