// GET /api/v1/export-invoices — paginated list for the /export-invoices list page.
// The invoice HEADER is created/updated through the transaction-pages runtime
// (POST /api/v1/pages/export-invoices/[id]); this route is read-only list.
...
export const GET = withErrorHandler(async (req: NextRequest) => {
  ...
  const { items, total } = await listInvoices('export', { q, status, page, pageSize });
