// §4.9 + §4.10 — filter-driven per-row bulk editor for Import Tracking (the PHP
...
//
// This is distinct from POST /api/imports/bulk (set one value across many rows).
// Here every row carries its own values, and the filter→predicate→fields mapping
// is data (master_bulk_filter_t), translated to safe whitelisted SQL.
...
const PAGE_SLUG = 'import';
const MAX_UPDATES = 500;

// Bulk access piggybacks on an EDIT grant on any import accordion (§4.7/§4.12).
async function hasImportEditAccess(roleId: number): Promise<boolean> {
  const rows = await db
...
// Field labels/types come from the import page's field config, so the editor
...
export async function GET(req: NextRequest) {
  ...
  if (!(await hasImportEditAccess(session.role_id))) return fail('Forbidden', 403);
  ...
  if (q) {
    ...
    where = sql`${where} AND (i.mca_ref ILIKE ${like} OR c.short_name ILIKE ${like}
      OR i.horse ILIKE ${like} OR i.trailer_1 ILIKE ${like} OR i.trailer_2 ILIKE ${like} OR i.container ILIKE ${like})`;
  }
  ...
  const countRes = await db.execute(
    sql`SELECT COUNT(*)::int AS total FROM imports_t i LEFT JOIN clients_t c ON c.id = i.client_id WHERE ${where}`,
  );
  const total = (countRes as unknown as { rows: { total: number }[] }).rows[0]?.total ?? 0;

  const offset = (page - 1) * pageSize;
  const dataRes = await db.execute(
    sql`SELECT i.id, i.mca_ref, i.pre_alert_date, c.short_name AS client_name${editableProjection}
        FROM imports_t i LEFT JOIN clients_t c ON c.id = i.client_id
        WHERE ${where}
        ORDER BY i.id ASC
...
export async function POST(req: NextRequest) {
  ...
  if (!session) return fail('Unauthorized', 401);
  if (!(await hasImportEditAccess(session.role_id))) return fail('Forbidden', 403);
  if (!getPageTarget(PAGE_SLUG)) return fail('Import page target not registered', 500);
    ...
    for (const u of updates) {
      ...
      const beforeRes = await tx.execute(
        sql`SELECT id, ${sql.join([...editable].map((c) => sql.identifier(c)), sql`, `)}
            FROM imports_t WHERE id = ${id} AND display = 'Y' LIMIT 1`,
      );
      ...
      await tx.execute(sql`UPDATE imports_t SET ${setSql} WHERE id = ${id}`);
