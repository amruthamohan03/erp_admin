// POST /api/imports/bulk — set one or more whitelisted fields on many imports at
// once. Authorization reuses the §4.12 model: the caller's role must hold an
// 'edit' grant on the 'import' page (so the same roles that can edit the form —
// e.g. Super Admin / Developer — can bulk-edit). Every row change is audited
// (§4.10) inside a single transaction.
...
import { BULK_FIELD_NAMES } from '@/lib/imports/bulkFields';
...
export async function POST(req: NextRequest) {
  ...
  // Authorization: caller's role must have an 'edit' grant on the import page.
  const grant = await db
    ...
    .limit(1);
  if (grant.length === 0) {
    return fail('Forbidden — your role cannot bulk-edit imports', 403);
  }

  // Restrict to (bulk whitelist ∩ real imports_t columns).
  const requested = Object.keys(values);
  const allowed = new Set(safeColumnsFor('import', BULK_FIELD_NAMES));
  ...
  const target = getPageTarget('import');
  if (!target) return fail('Import page target not registered', 500);

  const updated = await db.transaction(async (tx) => {
    let n = 0;
    for (const id of ids) {
      const before = await fetchEntityValues('import', id, cols);
