// GET /api/exports/export-by-client → multi-sheet Excel, one sheet per client
// (with a totals row), honouring the same filters as the /export list.
// "Client Excel" download in the legacy UI.
...
export async function GET(req: NextRequest) {
  ...
  const sheets = buildGroupedExportSheets(rows, 'client_name');
  const buf = await buildXlsx(sheets);
  return xlsxResponse(buf, `exports-by-client-${dateStamp()}.xlsx`);
}
