...
export async function PUT(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    if (d.currency_name !== undefined) patch.currencyName = d.currency_name;
    if (d.currency_short_name !== undefined) patch.currencyShortName = d.currency_short_name;
    ...
    const dup = uniqueViolationResponse(err, 'currency name');
    if (dup) return dup;
    // eslint-disable-next-line no-console
    console.error('[currencies.PUT]', err);
