...
const money = (x: number): string => x.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

// Option loader adapted to the restructure envelope: list endpoints return
// { ok, data: [...], meta }. pageSize is capped at 100 by every list query
// schema, so we request the max (main asked for 1000 against un-paginated
// legacy routes).
...
export default function QuotationsPage() {
  ...
  // Quotation items only appear once all four header keys are chosen (the same
  // combination that builds the reference).
  ...
  const descOptions = useCallback((catId: number): ItemOpt[] => {
  ...
  useEffect(() => {
    (async () => {
      const [cl, ki, tr, go, cu, un] = await Promise.all([
        ...
        fetchOpts('/api/v1/currencies?pageSize=100', (r) => String(r.currency_short_name ?? r.id)),
        fetchOpts('/api/v1/units?pageSize=100', (r) => String(r.unit_code || r.unit_name || r.id)),
      ]);
      setClients(cl); setKinds(ki); setTransports(tr); setGoods(go); setCurrencies(cu); setUnits(un);

      const catRes = await fetch('/api/v1/quotation-categories?pageSize=100'); const catJson = await catRes.json();
      if (catJson.ok) setCategories(catJson.data);
      const itRes = await fetch('/api/v1/items?pageSize=100'); const itJson = await itRes.json();
      if (itJson.ok) {
  ...
  useEffect(() => {
    (async () => {
      const [cRes, sRes] = await Promise.all([
        fetch('/api/v1/dashboard-cards/me').then((r) => r.json()).catch(() => null),
        fetch('/api/v1/quotations/stats').then((r) => r.json()).catch(() => null),
      ]);
      if (cRes?.ok) setCards((cRes.data as DashboardCard[]).filter((x) => x.card_category === 'quotation_dashboard'));
      if (sRes?.ok) setStats(sRes.data as Record<string, number>);
    })();
  }, []);
  ...
  const rowTotals = useCallback((r: Row, isCustomsCat: boolean) => {
  ...
  }, [rows, sortedCategories, isImportDefinitive, arsp, rowTotals]);
  ...
  async function loadForEdit(id: number, copy = false) {
    const res = await fetch(`/api/v1/quotations/${id}`); const j = await res.json();
    if (!j.ok) { setErr('Failed to load quotation'); return; }
    const q = j.data.header as Record<string, unknown>;
    const its = j.data.items as Array<Record<string, unknown>>;
    setFormOpen(true);
    setQuotationId(copy ? null : (q.id as number));
    ...
    setQuotationDate(copy ? new Date().toISOString().slice(0, 10) : ((q.quotation_date as string) ?? new Date().toISOString().slice(0, 10)));
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const url = quotationId ? `/api/v1/quotations/${quotationId}` : '/api/v1/quotations';
      const res = await fetch(url, { method: quotationId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const j = await res.json();
      if (!res.ok || !j.ok) { setErr(j.error?.message || 'Save failed'); return; }
