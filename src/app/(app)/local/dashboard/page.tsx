    ...
    <div className="space-y-2">
      {data.map((d, i) => (
        <div key={d.label + i} className="flex items-center gap-2">
          <span className="w-28 text-xs text-muted-foreground text-right truncate" title={d.label}>{d.label}</span>
    ...
    <>
      <div className="card p-5 mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Local Tracking Dashboard</h1>
      ...
      {!loading && d && (
        <>
          ...
          <div className="card p-4 mb-5">
            <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2"><MapPin className="h-4 w-4" /> Top Locations</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-5">
            <div className="card p-4">
              <h3 className="font-semibold text-foreground mb-3">Location Distribution</h3>
              <HBars data={d.location_distribution.map((l) => ({ label: l.location_name, value: l.tracking_count }))} />
            </div>
            <div className="card p-4">
              <h3 className="font-semibold text-foreground mb-3">Client Types</h3>
          ...
          {/* Monthly trend */}
          <div className="card p-4 mb-5">
            <h3 className="font-semibold text-foreground mb-3">Monthly Trend (Last 12 Months)</h3>
              ...
              <div className="flex items-end gap-2 h-40">
                {d.monthly_trend.map((m) => (
                  <div key={m.month_name} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                    <div className="text-[10px] text-muted-foreground">{m.tracking_count}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-5">
            <div className="card p-4">
              <h3 className="font-semibold text-foreground mb-3">Top 10 Horses</h3>
              <HBars data={d.horse_performance.map((h) => ({ label: h.horse_name, value: h.trip_count }))} />
            </div>
            <div className="card p-4">
              <h3 className="font-semibold text-foreground mb-3">Top 10 Trailers</h3>
          ...
          <div className="card mb-5">
            <div className="px-4 py-3 border-b border-border font-semibold text-foreground">Top 10 Clients</div>
              ...
              <table className="table-base">
                ...
                <tbody>
                  ...
                  {d.top_clients.map((c, i) => (
                    <tr key={c.short_name + i} className="hover:bg-muted/50">
          ...
          {/* Recent trackings */}
          <div className="card">
            <div className="px-4 py-3 border-b border-border font-semibold text-foreground">Recent Trackings</div>
            <div className="overflow-x-auto">
              <table className="table-base whitespace-nowrap">
                ...
                <tbody>
                  ...
                  {d.recent_trackings.map((t) => {
                    ...
                    return (
                      <tr key={String(t.id)} className="hover:bg-muted/50 text-xs">
                        ...
                        <td>
                          {ceecOut
                            ? <span className="inline-block rounded-full bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 px-2 py-0.5">Done ({dur ?? 0} days)</span>
                            : ceecIn
                              ? <span className="inline-block rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 px-2 py-0.5">In Progress</span>
                              : <span className="inline-block rounded-full bg-muted text-muted-foreground px-2 py-0.5">Pending</span>}
                        </td>
                      </tr>
                    );
                  })}
