...
// Import Delay KPI — read-only analytics over import milestones. All figures
// from /api/v1/imkpi; drill-down from /api/v1/imkpi/stage-records.
...
interface ClientRow {
  ...
  avg_total: number | null; avg_pre_zambia: number | null; avg_zambia_wait: number | null; avg_drc_border: number | null;
  avg_border_wait: number | null; avg_customs: number | null; avg_dgda_liquid: number | null; avg_liquid_quittance: number | null;
}
...
// SVG progress ring for priority cards.
  ...
  return (
    <div className="relative w-[88px] h-[88px] shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-[9px] font-bold uppercase tracking-wide text-slate-400">days</span>
      </div>
    </div>
  );
}
...
export default function ImkpiPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/imkpi?${p}`);
  ...
  const loadRecords = useCallback(async (stage: string, sf: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/imkpi/stage-records?${p}`);
      const j = await res.json();
  ...
  const total = s.total_imports ?? 0;
  const today = new Date().toISOString().slice(0, 10);

  const summaryCards = [
    { label: 'Total Imports', value: num(total), sub: 'All records', icon: <FileInput className="h-5 w-5" />, grad: 'from-indigo-500 to-violet-600' },
    ...
    { label: 'On Time ≤21d', value: num(s.on_time_total ?? 0), sub: `${pct(s.on_time_total ?? 0, total)}% of total`, icon: <CircleCheck className="h-5 w-5" />, grad: 'from-emerald-500 to-teal-600' },
    { label: 'Delayed >21d', value: num(s.delayed_total ?? 0), sub: `${pct(s.delayed_total ?? 0, total)}% of total`, icon: <AlertOctagon className="h-5 w-5" />, grad: 'from-rose-500 to-red-600' },
    { label: 'Avg Total', value: `${d1(s.avg_total_days ?? 0)}d`, sub: 'Pre Alert → Deliver', icon: <Clock className="h-5 w-5" />, grad: 'from-cyan-500 to-sky-600' },
  ];
  ...
  return (
    <>
      ...
      <div className="card overflow-hidden mb-4">
        ...
        <div className="p-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Import Delay KPI</h1>
              <p className="text-xs text-slate-500">On Time vs Delayed · Working days · Pending aged to today ({today})</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/imports" className="btn-secondary"><FileInput className="h-4 w-4" /> Imports</Link>
      ...
      {loading && <div className="card p-6 text-center text-slate-500">Loading KPIs…</div>}
      ...
      {!loading && data && (
        <>
                    ...
                    <div key={pk.key} className={`card p-4 border-l-4 ${over ? 'border-rose-500' : 'border-emerald-500'}`}>
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <div className="rounded-lg border-l-2 border-emerald-500 bg-slate-50 dark:bg-slate-800/40 px-3 py-2">
                          ...
                          <div className="text-[10px] uppercase font-bold text-slate-400">On Time ≤{pk.threshold}d</div>
                        ...
                        <div className="rounded-lg border-l-2 border-rose-500 bg-slate-50 dark:bg-slate-800/40 px-3 py-2">
                          ...
                          <div className="text-[10px] uppercase font-bold text-slate-400">Delayed &gt;{pk.threshold}d</div>
          ...
          {/* Stage rows */}
          <h2 className="text-xs font-bold uppercase tracking-widest text-blue-600 mb-2 flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-blue-500" /> Stage-by-Stage Delay Analysis <span className="text-slate-400 normal-case font-normal tracking-normal">· click to drill down</span></h2>
              ...
              return (
                <button key={k.key} type="button" onClick={() => openDrill(k)}
                  ...
                  <div className="hidden lg:flex w-52 shrink-0 p-3 border-r border-slate-100 dark:border-slate-800 flex-col justify-center gap-1 text-[11px]">
                    <div><span className="text-slate-400 font-bold w-8 inline-block">FROM</span> <span className="font-mono text-slate-700 dark:text-slate-200">{fl(k.from)}</span></div>
                    <div><span className="text-slate-400 font-bold w-8 inline-block">TO</span> <span className="font-mono text-slate-700 dark:text-slate-200">{fl(k.to)}</span></div>
                  </div>
                  <div className="w-28 shrink-0 p-3 border-r border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center">
                    ...
                    <span className="text-[10px] uppercase font-bold text-slate-400">avg days</span>
                  ...
                  <div className="flex-1 p-3 flex flex-col justify-center gap-1.5 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-bold uppercase text-slate-400">{num(k.evaluated_count)} records evaluated</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-5">
            <div className="card p-4">
              <h3 className="font-semibold text-slate-800 dark:text-slate-200 mb-3 flex items-center gap-2"><AlertOctagon className="h-4 w-4 text-rose-500" /> Top Bottlenecks <span className="text-xs text-slate-400 font-normal">by % delayed</span></h3>
                ...
                {data.bottleneck_analysis.slice(0, 11).map((b, i) => (
                  <div key={b.key} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-slate-400">{num(b.sample_count)} records · target ≤{b.threshold}d · {num(b.delayed_count)} delayed</div>
                    </div>
            ...
            <div className="card p-4">
              <h3 className="font-semibold text-slate-800 dark:text-slate-200 mb-3 flex items-center gap-2"><CalendarOff className="h-4 w-4 text-amber-500" /> DRC Public Holidays <span className="text-xs text-slate-400 font-normal">excluded from delay days (Sat/Sun too)</span></h3>
              <div className="grid grid-cols-2 gap-2 max-h-[340px] overflow-y-auto">
                {holYear.length === 0 && <p className="text-sm text-slate-400 col-span-2 py-4 text-center">No holidays for {today.slice(0, 4)}</p>}
                {holYear.map((h) => {
                  const passed = h.holiday_date < today;
                  const [, m, dd] = h.holiday_date.split('-');
                  ...
                  return (
                    <div key={h.holiday_date} className={`flex items-center gap-2 rounded-lg border border-slate-200 dark:border-slate-700 px-2 py-1.5 ${passed ? 'opacity-50' : 'border-l-2 border-l-emerald-500'}`}>
                      <div className="shrink-0 w-10 text-center rounded bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-0.5">
                        ...
                        <div className="text-[9px] font-bold text-slate-400 uppercase">{mon}</div>
                      </div>
                      <div className="min-w-0">
                        <div className="text-xs font-semibold text-slate-800 dark:text-slate-100 truncate">{h.name_en}</div>
                        <div className="text-[10px] text-slate-400 truncate">{h.name_fr}</div>
                      </div>
              ...
              <table className="table-base whitespace-nowrap text-xs">
                <thead>
                  <tr>
                    ...
                    <th className="text-center">Avg Total</th><th className="text-center">Pre→Zambia</th><th className="text-center">Zambia Wait</th>
                    <th className="text-center">DRC→WH</th><th className="text-center">Border Wait</th><th className="text-center">Customs</th>
                    <th className="text-center">DGDA→Liq</th><th className="text-center">Liq→Quitt</th>
                  </tr>
                </thead>
                <tbody>
                  {data.client_delay_table.length === 0 && <tr><td colSpan={13} className="text-center text-slate-500 py-6">No data available</td></tr>}
                  {data.client_delay_table.map((r, i) => (
                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      <td className="font-semibold text-slate-900 dark:text-slate-100">{r.client_name}</td>
                      <td className="text-center font-mono">{num(r.total_imports)}</td>
                      <td className="text-center"><span className="rounded bg-blue-50 text-blue-700 px-2 py-0.5 font-bold">{num(r.delivered_count)}</span></td>
                      <td className="text-center"><span className="rounded bg-emerald-100 text-emerald-700 px-2 py-0.5 font-bold">{num(r.on_time_count)}</span></td>
                      <td className="text-center"><span className="rounded bg-rose-100 text-rose-700 px-2 py-0.5 font-bold">{num(r.delayed_count)}</span></td>
                      <td className="text-center"><DP v={r.avg_total} th={21} /></td>
                      <td className="text-center"><DP v={r.avg_pre_zambia} th={7} /></td>
                      <td className="text-center"><DP v={r.avg_zambia_wait} th={3} /></td>
                      <td className="text-center"><DP v={r.avg_drc_border} th={3} /></td>
                      <td className="text-center"><DP v={r.avg_border_wait} th={3} /></td>
                      <td className="text-center"><DP v={r.avg_customs} th={5} /></td>
                      <td className="text-center"><DP v={r.avg_dgda_liquid} th={2} /></td>
                      <td className="text-center"><DP v={r.avg_liquid_quittance} th={2} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="card w-full max-w-6xl my-auto overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4">
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                <span className="ml-auto text-xs text-slate-400 self-center">{records.length} record{records.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="overflow-x-auto max-h-[62vh]">
                {recLoading ? <div className="py-10 text-center text-slate-500">Loading records…</div> : (
                  <table className="table-base whitespace-nowrap text-xs">
                    <thead>
                    ...
                    <tbody>
                      {records.length === 0 && <tr><td colSpan={11} className="text-center text-slate-500 py-6">No records found</td></tr>}
                      {records.map((r, idx) => {
                        ...
                        return (
                          <Fragment key={idx}>
                            ...
                            <td className="text-slate-400">{expanded === idx ? '▾' : '▸'}</td>
                            <td className="font-mono text-slate-400">{String(r.id)}</td>
                            ...
                            <tr className="bg-slate-50 dark:bg-slate-800/40">
                              <td colSpan={11} className="p-3">
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-1 text-[11px]">
                                  {[['Supplier', r.supplier], ['License', r.license_number], ['Decl. Office', r.declaration_office], ['Goods', r.goods_type], ['Kind', r.kind_name], ['Transport', r.transport_mode], ['Weight', `${r.weight} kg`], ['Status', r.clearing_status],
                                    ['Pre Alert', r.pre_alert_date], ['Arr Zambia', r.arrival_date_zambia], ['DRC Entry', r.drc_entry_date], ['Border WH', r.border_warehouse_arrival_date], ['Disp Border', r.dispatch_from_border], ['Deliver', r.dispatch_deliver_date], ['DGDA In', r.dgda_in_date], ['DGDA Out', r.dgda_out_date], ['Liquidation', r.liquidation_date], ['Quittance', r.quittance_date]].map(([lbl, v]) => (
                                    <div key={String(lbl)} className="flex justify-between gap-2"><span className="text-slate-400">{String(lbl)}</span><span className="font-medium text-slate-800 dark:text-slate-100 truncate">{v == null || v === '' ? '—' : String(v)}</span></div>
                                  ))}
                                </div>
                              </td>
                            </tr>
            ...
            <div className="flex justify-end gap-2 border-t border-slate-200 dark:border-slate-800 px-5 py-3">
              <button type="button" disabled title="Excel export — coming in the next pass" className="inline-flex items-center gap-1.5 rounded-md bg-slate-200 text-slate-400 px-3 py-1.5 text-sm font-medium cursor-not-allowed dark:bg-slate-800"><Download className="h-4 w-4" /> Export</button>
