...
export default function PhasesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/phases?${params}`);
  ...
  async function handleDelete(id: number) {
    ...
    const res = await fetch(`/api/v1/phases/${id}`, { method: 'DELETE' });
    const json = await res.json();
    if (!json.ok) {
      setResult({ status: 'error', title: 'Not deleted', message: json.error?.message || 'This phase could not be disabled.' });
      return;
    }
    setResult({ status: 'success', title: 'Deleted', message: 'The phase has been disabled.' });
    load();
  }
      ...
      {showCreate && (
        <FormModal
          ...
          onSaved={() => {
            setShowCreate(false);
            load();
            setResult({ status: 'success', title: 'Created', message: 'The phase has been created.' });
          }}
        />
      )}
      ...
      {editing && (
        <FormModal
          ...
          onSaved={() => {
            setEditing(null);
            load();
            setResult({ status: 'success', title: 'Saved', message: 'Your changes to this phase have been saved.' });
          }}
        />
      )}
...
function FormModal({
  ...
  const isEdit = !!row;
  const [name, setName] = useState(row?.phase_name || '');
  const [code, setCode] = useState(row?.phase_code || '');
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/v1/phases/${row!.id}` : '/api/v1/phases';
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ phase_name: name, phase_code: code }),
      });
      const json = await res.json();
      if (!res.ok || !json.ok) {
