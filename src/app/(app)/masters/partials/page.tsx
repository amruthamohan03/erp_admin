...
export default function PartialsPage() {
  const [items, setItems] = useState<PartialRow[]>([]);
  ...
  const [editing, setEditing] = useState<PartialRow | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/partials?${params}`);
  ...
  async function handleDelete(id: number) {
    if (!confirm('Disable this partial?')) return;
    const res = await fetch(`/api/v1/partials/${id}`, { method: 'DELETE' });
    const json = await res.json();
    if (!json.ok) {
    ...
    <>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowCreate(true)} className="btn-primary">
          <Plus className="h-4 w-4" /> New Partial
        </button>
      </div>
        ...
        <div className="p-4 border-b border-slate-200">
          <div className="relative max-w-sm">
            ...
            <input
              ...
              placeholder="Search partial name..."
              value={search}
              onChange={(e) => {
...
function PartialFormModal({
  ...
  const isEdit = !!partial;
  const [name, setName] = useState(partial?.partial_name || '');
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/v1/partials/${partial!.id}` : '/api/v1/partials';
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ partial_name: name }),
      });
      const json = await res.json();
      if (!res.ok || !json.ok) {
      ...
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">
            {isEdit ? 'Edit Partial' : 'Create Partial'}
          </h2>
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            <label className="label">Partial Name *</label>
            <input
              className="input"
              value={name}
