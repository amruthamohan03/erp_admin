...
export default function GoodsTypesPage() {
  const [items, setItems] = useState<GoodsTypeRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/goods-types?${params}`);
...
function GoodsTypeFormModal({
  goodsType,
  onClose,
  onSaved,
}: {
  goodsType?: GoodsTypeRow;
  onClose: () => void;
  onSaved: () => void;
}) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit
      ? `/api/v1/goods-types/${goodsType!.id}`
      : '/api/v1/goods-types';
