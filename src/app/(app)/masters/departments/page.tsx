...
export default function DepartmentsPage() {
  ...
  async function handleDelete(id: number) {
    ...
    const json = await res.json();
    if (!json.ok) {
      setResult({ status: 'error', title: 'Not deleted', message: json.error?.message || 'This department could not be disabled.' });
      return;
    }
    setResult({ status: 'success', title: 'Deleted', message: 'The department has been disabled.' });
    load();
  }
              ...
              {!loading &&
                items.map((r, idx) => (
                  <tr key={r.id} className="hover:bg-slate-50">
                    ...
                    <td className="font-medium">{r.department_name}</td>
      ...
      {showCreate && (
        <FormModal
          ...
          onSaved={() => {
            setShowCreate(false);
            load();
            setResult({ status: 'success', title: 'Created', message: 'The department has been created.' });
          }}
        />
      )}
      ...
      {editing && (
        <FormModal
          ...
          onSaved={() => {
            setEditing(null);
            load();
            setResult({ status: 'success', title: 'Saved', message: 'Your changes to this department have been saved.' });
          }}
        />
      )}
...
function FormModal({
  ...
  const isEdit = !!row;
  const [name, setName] = useState(row?.department_name || '');
