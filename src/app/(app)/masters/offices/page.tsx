...
export default function OfficesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/offices?${params}`);
  ...
  async function handleDelete(id: number) {
    if (!confirm('Disable this office? References from seal batches stay intact.')) return;
    const res = await fetch(`/api/v1/offices/${id}`, { method: 'DELETE' });
    const json = await res.json();
    if (!json.ok) {
...
function OfficeFormModal({
  ...
  const isEdit = !!office;
  const [locationName, setLocationName] = useState(office?.location_name || '');
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/v1/offices/${office!.id}` : '/api/v1/offices';
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">{isEdit ? 'Edit Office' : 'Create Office'}</h2>
