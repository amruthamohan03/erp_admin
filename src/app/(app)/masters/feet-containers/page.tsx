...
export default function FeetContainersPage() {
  ...
  async function handleDelete(id: number) {
    ...
    const json = await res.json();
    if (!json.ok) {
      setResult({ status: 'error', title: 'Not deleted', message: json.error?.message || 'This container size could not be disabled.' });
      return;
    }
    setResult({ status: 'success', title: 'Deleted', message: 'The container size has been disabled.' });
    load();
  }
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-foreground">Feet / Container Sizes</h1>
      ...
      <DataTable<Row>
        ...
        emptyMessage="No container sizes yet — create the first one."
        columns={[
        { key: 'feet_container_size', header: 'Size', sortable: true, className: 'font-medium' },
        ]}
      ...
      {showCreate && (
        <FormModal
          ...
          onSaved={() => {
            setShowCreate(false);
            load();
            setResult({ status: 'success', title: 'Created', message: 'The container size has been created.' });
          }}
        />
      )}
      ...
      {editing && (
        <FormModal
          ...
          onSaved={() => {
            setEditing(null);
            load();
            setResult({ status: 'success', title: 'Saved', message: 'Your changes to this container size have been saved.' });
          }}
        />
      )}
