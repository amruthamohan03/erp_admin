...
import type { ClearingStatus } from '@/types';

export default function ClearingStatusesPage() {
  const [items, setItems] = useState<ClearingStatus[]>([]);
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(false);
  const [showCreate, setShowCreate] = useState(false);
  const [editing, setEditing] = useState<ClearingStatus | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/clearing-statuses');
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((c) => c.clearing_status?.toLowerCase().includes(q));
  }, [items, search]);
  ...
  async function handleDelete(id: number) {
    ...
    const res = await fetch(`/api/clearing-statuses/${id}`, { method: 'DELETE' });
    const json = await res.json();
  ...
  return (
    <DashboardShell>
      ...
      <div className="card">
        <div className="p-4 border-b border-slate-200">
          <div className="relative max-w-sm">
            ...
            <input className="input pl-9" placeholder="Search clearing status..." value={search}
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              ...
              {!loading && paged.length === 0 && (<tr><td colSpan={3} className="text-center text-slate-500 py-8">No clearing statuses found</td></tr>)}
      ...
      {showCreate && (<ClearingStatusFormModal onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); load(); }} />)}
      {editing && (<ClearingStatusFormModal item={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />)}
    </DashboardShell>
  );
}
...
function ClearingStatusFormModal({
...
}: { item?: ClearingStatus; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  const [form, setForm] = useState({ clearing_status: item?.clearing_status || '' });
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const unique = useUniqueCheck({
    endpoint: '/api/uniqueness/clearing-statuses',
    value: form.clearing_status,
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/clearing-statuses/${item!.id}` : '/api/clearing-statuses';
    const method = isEdit ? 'PUT' : 'POST';
    try {
      const res = await fetch(url, {
        method, headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clearing_status: form.clearing_status }),
      });
      const json = await res.json();
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">{isEdit ? 'Edit Clearing Status' : 'Create Clearing Status'}</h2>
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            ...
            <input className="input uppercase" value={form.clearing_status}
              onChange={(e) => setForm({ clearing_status: e.target.value.toUpperCase() })} required maxLength={100} />
