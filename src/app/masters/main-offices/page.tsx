...
import type { MainOffice } from '@/types';

export default function MainOfficesPage() {
  const [items, setItems] = useState<MainOffice[]>([]);
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(false);
  const [showCreate, setShowCreate] = useState(false);
  const [editing, setEditing] = useState<MainOffice | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/main-offices');
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((i) => (i.main_location_name ?? '').toLowerCase().includes(q));
  }, [items, search]);
  ...
  async function handleDelete(id: number) {
    if (!confirm('Disable this main office?')) return;
    const res = await fetch(`/api/main-offices/${id}`, { method: 'DELETE' });
    const json = await res.json();
  ...
  return (
    <DashboardShell>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Main Offices</h1>
      ...
      <div className="card">
        <div className="p-4 border-b border-slate-200">
          <div className="relative max-w-sm">
            ...
            <input className="input pl-9" placeholder="Search main location name..." value={search}
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              ...
              {!loading && paged.length === 0 && (<tr><td colSpan={3} className="text-center text-slate-500 py-8">No main offices found</td></tr>)}
              {!loading && paged.map((i, idx) => (
                <tr key={i.id} className="hover:bg-slate-50">
                  <td className="text-slate-500 font-medium">{startIndex + idx + 1}</td>
                  <td className="font-medium">{i.main_location_name || '—'}</td>
      ...
      {showCreate && (<MainOfficeFormModal onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); load(); }} />)}
      {editing && (<MainOfficeFormModal item={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />)}
    </DashboardShell>
  );
}
...
function MainOfficeFormModal({
...
}: { item?: MainOffice; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  const [form, setForm] = useState({ main_location_name: item?.main_location_name || '' });
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const unique = useUniqueCheck({
    endpoint: '/api/uniqueness/main-offices',
    value: form.main_location_name,
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/main-offices/${item!.id}` : '/api/main-offices';
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">{isEdit ? 'Edit Main Office' : 'Create Main Office'}</h2>
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            ...
            <input className="input uppercase" value={form.main_location_name}
              onChange={(e) => setForm({ main_location_name: e.target.value.toUpperCase() })} required maxLength={255} />
