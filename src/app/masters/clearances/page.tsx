...
export default function ClearancesPage() {
  const [items, setItems] = useState<Clearance[]>([]);
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((c) => c.clearance_name?.toLowerCase().includes(q));
  }, [items, search]);
  ...
  return (
    <DashboardShell>
      ...
      <div className="card">
        <div className="p-4 border-b border-slate-200">
          <div className="relative max-w-sm">
            ...
            <input className="input pl-9" placeholder="Search clearance name..." value={search}
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              ...
              {!loading && paged.length === 0 && (<tr><td colSpan={3} className="text-center text-slate-500 py-8">No clearances found</td></tr>)}
              {!loading && paged.map((c, idx) => (
                <tr key={c.id} className="hover:bg-slate-50">
                  <td className="text-slate-500 font-medium">{startIndex + idx + 1}</td>
                  <td className="font-medium">{c.clearance_name}</td>
      ...
      {showCreate && (<ClearanceFormModal onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); load(); }} />)}
      {editing && (<ClearanceFormModal item={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />)}
    </DashboardShell>
  );
}
...
function ClearanceFormModal({
...
}: { item?: Clearance; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  const [form, setForm] = useState({ clearance_name: item?.clearance_name || '' });
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const unique = useUniqueCheck({
    endpoint: '/api/uniqueness/clearances',
    value: form.clearance_name,
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/clearances/${item!.id}` : '/api/clearances';
    const method = isEdit ? 'PUT' : 'POST';
    try {
      const res = await fetch(url, {
        method, headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clearance_name: form.clearance_name }),
      });
      const json = await res.json();
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">{isEdit ? 'Edit Clearance' : 'Create Clearance'}</h2>
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            ...
            <input className="input uppercase" value={form.clearance_name}
              onChange={(e) => setForm({ clearance_name: e.target.value.toUpperCase() })} required maxLength={255} />
