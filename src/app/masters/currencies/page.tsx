  ...
  const filtered = useMemo(() => {
    ...
    return currencies.filter(
      (c) =>
        ...
        c.currency_short_name?.toLowerCase().includes(q),
    );
  }, [currencies, search]);
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              ...
              {!loading && paged.length === 0 && (<tr><td colSpan={4} className="text-center text-slate-500 py-8">No currencies found</td></tr>)}
...
function CurrencyFormModal({
  ...
  const unique = useUniqueCheck({
    endpoint: '/api/uniqueness/currencies',
    value: form.currency_name,
    excludeId: currency?.id ?? null,
  });
  ...
  async function submit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      currency_short_name: form.currency_short_name,
    };
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">{isEdit ? 'Edit Currency' : 'Create Currency'}</h2>
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            ...
            <input className="input uppercase" value={form.currency_name}
              onChange={(e) => setForm({ ...form, currency_name: e.target.value.toUpperCase() })} required maxLength={100} />
            <UniqueAvailability status={unique.status} message={unique.message} />
          </div>
          <div>
            <label className="label">Short Name *</label>
            <input className="input uppercase" value={form.currency_short_name}
              onChange={(e) => setForm({ ...form, currency_short_name: e.target.value.toUpperCase() })} required maxLength={10} />
          </div>
