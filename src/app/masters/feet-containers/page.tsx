...
import type { FeetContainer } from '@/types';

export default function FeetContainersPage() {
  const [items, setItems] = useState<FeetContainer[]>([]);
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(false);
  const [showCreate, setShowCreate] = useState(false);
  const [editing, setEditing] = useState<FeetContainer | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/feet-containers');
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((i) => i.feet_container_size?.toLowerCase().includes(q));
  }, [items, search]);
  ...
  async function handleDelete(id: number) {
    ...
    const res = await fetch(`/api/feet-containers/${id}`, { method: 'DELETE' });
    const json = await res.json();
  ...
  return (
    <DashboardShell>
      ...
      <div className="card">
        <div className="p-4 border-b border-slate-200">
          <div className="relative max-w-sm">
            ...
            <input className="input pl-9" placeholder="Search container size..." value={search}
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              ...
              {!loading && paged.length === 0 && (<tr><td colSpan={3} className="text-center text-slate-500 py-8">No sizes found</td></tr>)}
              {!loading && paged.map((i, idx) => (
                <tr key={i.id} className="hover:bg-slate-50">
                  <td className="text-slate-500 font-medium">{startIndex + idx + 1}</td>
                  <td className="font-medium">{i.feet_container_size}</td>
      ...
      {showCreate && (<FeetContainerFormModal onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); load(); }} />)}
      {editing && (<FeetContainerFormModal item={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />)}
    </DashboardShell>
  );
}

function FeetContainerFormModal({
  item, onClose, onSaved,
}: { item?: FeetContainer; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  const [form, setForm] = useState({ feet_container_size: item?.feet_container_size || '' });
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const unique = useUniqueCheck({
    endpoint: '/api/uniqueness/feet-containers',
    value: form.feet_container_size,
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/feet-containers/${item!.id}` : '/api/feet-containers';
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">{isEdit ? 'Edit Container Size' : 'Create Container Size'}</h2>
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            ...
            <input className="input uppercase" value={form.feet_container_size}
              onChange={(e) => setForm({ feet_container_size: e.target.value.toUpperCase() })} required maxLength={50} />
