...
import type { Referer } from '@/types';

export default function ReferersPage() {
  const [items, setItems] = useState<Referer[]>([]);
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(false);
  const [showCreate, setShowCreate] = useState(false);
  const [editing, setEditing] = useState<Referer | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/referers');
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((i) => i.refferer_name?.toLowerCase().includes(q));
  }, [items, search]);
  ...
  async function handleDelete(id: number) {
    if (!confirm('Disable this referrer?')) return;
    const res = await fetch(`/api/referers/${id}`, { method: 'DELETE' });
    const json = await res.json();
  ...
  return (
    <DashboardShell>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Referrers</h1>
        <button onClick={() => setShowCreate(true)} className="btn-primary">
          <Plus className="h-4 w-4" /> New Referrer
        </button>
      </div>
      ...
      <div className="card">
        <div className="p-4 border-b border-slate-200">
          <div className="relative max-w-sm">
            ...
            <input className="input pl-9" placeholder="Search referrer name..." value={search}
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              ...
              {!loading && paged.length === 0 && (<tr><td colSpan={3} className="text-center text-slate-500 py-8">No referrers found</td></tr>)}
              {!loading && paged.map((i, idx) => (
                <tr key={i.id} className="hover:bg-slate-50">
                  <td className="text-slate-500 font-medium">{startIndex + idx + 1}</td>
                  <td className="font-medium">{i.refferer_name}</td>
      ...
      {showCreate && (<RefererFormModal onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); load(); }} />)}
      {editing && (<RefererFormModal item={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />)}
    </DashboardShell>
  );
}

function RefererFormModal({
  item, onClose, onSaved,
}: { item?: Referer; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  const [form, setForm] = useState({ refferer_name: item?.refferer_name || '' });
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const unique = useUniqueCheck({
    endpoint: '/api/uniqueness/referers',
    value: form.refferer_name,
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/referers/${item!.id}` : '/api/referers';
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">{isEdit ? 'Edit Referrer' : 'Create Referrer'}</h2>
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            ...
            <input className="input" value={form.refferer_name}
              onChange={(e) => setForm({ refferer_name: e.target.value })} required maxLength={255} />
