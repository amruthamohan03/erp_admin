...
export default function MasterPagesList() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setLoadError(null);
    const result = await safeFetchJson<MasterPage[]>('/api/master-pages');
    if (result.ok) {
  ...
  async function handleDelete(id: number) {
    if (!confirm('Disable this page? Existing references to it will still work until the runtime stops finding it.')) return;
    ...
    <DashboardShell>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Configure §4.12 pages — slug, title, target table, accordions, role grants, and fields.</p>
        </div>
      ...
      <div className="card">
        <div className="p-4 border-b border-slate-200">
          <div className="relative max-w-sm">
            ...
            <input className="input pl-9" placeholder="Search slug, title, target table..." value={search}
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              ...
              {!loading && paged.length === 0 && (<tr><td colSpan={7} className="text-center text-slate-500 py-8">No pages found — click <strong>New Page</strong>.</td></tr>)}
              {!loading && paged.map((i, idx) => (
                <tr key={i.id} className="hover:bg-slate-50">
                  ...
                  <td>
                    <span className={`text-[10px] uppercase rounded px-1.5 py-0.5 ${i.display === 'Y' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                      {i.display === 'Y' ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="text-right">
                    <Link href={`/masters/pages/${i.id}${editSuffix}`} className="text-slate-500 hover:text-primary-600 p-1 inline-block" title="Configure">
      ...
      {showCreate && (<CreatePageModal onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); load(); }} />)}
    </DashboardShell>
  );
}
...
function CreatePageModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const unique = useUniqueCheck({
    endpoint: '/api/uniqueness/master-pages-slug',
    value: form.slug,
    excludeId: null,
  });
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (unique.status === 'taken') { setError('Slug already exists'); return; }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            <label className="label">Slug * <span className="text-xs text-slate-500 ml-1">(URL key, lowercase + hyphens)</span></label>
          ...
          <div>
            ...
            <input className="input font-mono" value={form.route} placeholder="/example"
              onChange={(e) => setForm({ ...form, route: e.target.value })} required maxLength={200} />
          </div>
          <div>
            <label className="label">Target Table * <span className="text-xs text-slate-500 ml-1">(must be in src/lib/pages/targets.ts whitelist)</span></label>
            <input className="input font-mono" value={form.target_table} placeholder="clients_t"
