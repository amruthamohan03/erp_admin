...
export default function TransportModesPage() {
  const [items, setItems] = useState<TransportMode[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/transport-modes');
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (m) =>
        m.transport_mode_name?.toLowerCase().includes(q) ||
        m.transport_letter?.toLowerCase().includes(q),
    );
  }, [items, search]);
  ...
  async function handleDelete(id: number) {
    ...
    const res = await fetch(`/api/transport-modes/${id}`, { method: 'DELETE' });
    const json = await res.json();
  ...
  return (
    <DashboardShell>
      ...
      <div className="card">
        <div className="p-4 border-b border-slate-200">
          <div className="relative max-w-sm">
            ...
            <input className="input pl-9" placeholder="Search mode name, letter..." value={search}
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            ...
            <tbody>
              ...
              {!loading && paged.length === 0 && (<tr><td colSpan={4} className="text-center text-slate-500 py-8">No transport modes found</td></tr>)}
              {!loading && paged.map((m, idx) => (
                <tr key={m.id} className="hover:bg-slate-50">
                  <td className="text-slate-500 font-medium">{startIndex + idx + 1}</td>
                  <td className="font-medium">{m.transport_mode_name}</td>
                  <td>
                  ...
                  <td className="text-right">
                    <button onClick={() => setEditing(m)} className="text-slate-500 hover:text-primary-600 p-1" title="Edit">
                      <Edit2 className="h-4 w-4" />
                    </button>
                    <button onClick={() => handleDelete(m.id)} className="text-slate-500 hover:text-red-600 p-1 ml-1" title="Disable">
      ...
      {showCreate && (<TransportModeFormModal onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); load(); }} />)}
      {editing && (<TransportModeFormModal item={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />)}
    </DashboardShell>
  );
}
...
function TransportModeFormModal({
...
}: { item?: TransportMode; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  const [form, setForm] = useState({
    transport_mode_name: item?.transport_mode_name || '',
    transport_letter: item?.transport_letter || '',
  });
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const unique = useUniqueCheck({
    endpoint: '/api/uniqueness/transport-modes',
    value: form.transport_mode_name,
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = isEdit ? `/api/transport-modes/${item!.id}` : '/api/transport-modes';
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="font-semibold">{isEdit ? 'Edit Transport Mode' : 'Create Transport Mode'}</h2>
        ...
        <form onSubmit={submit} className="p-4 space-y-3">
          ...
          <div>
            ...
            <input className="input uppercase" value={form.transport_mode_name}
              onChange={(e) => setForm({ ...form, transport_mode_name: e.target.value.toUpperCase() })} required maxLength={100} />
            <UniqueAvailability status={unique.status} message={unique.message} />
          </div>
          <div>
            <label className="label">Transport Letter *</label>
            <input className="input uppercase" value={form.transport_letter}
              onChange={(e) => setForm({ ...form, transport_letter: e.target.value.toUpperCase() })} required maxLength={5} />
          </div>
