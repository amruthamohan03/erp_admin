...
// §4.9 — filter-driven bulk editor for Export Tracking. Pick a "pending/missing"
...
export default function ExportBulkPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      if (q) params.set('q', q);
      const res = await fetch(`/api/exports/bulk-edit?${params.toString()}`);
  ...
  async function save() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/exports/bulk-edit', {
        method: 'POST',
  ...
  return (
    <DashboardShell>
      <div className="mb-4">
        <BackButton fallback="/export" />
      </div>
      <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-slate-900">Bulk Update — Export Tracking</h1>
        {dirtyCount > 0 && (
        ...
        <div className="flex-1 min-w-[220px]">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search MCA ref, client, horse, trailer, wagon, container…"
              value={q}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-slate-600">
                ...
                <th className="px-3 py-2">Loading Date</th>
                {fields.map((f) => (
            ...
            <tbody>
              ...
              {!loading && items.map((row, idx) => (
                <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-3 py-2">{asStr(row.loading_date) || '—'}</td>
                  {fields.map((f) => (
