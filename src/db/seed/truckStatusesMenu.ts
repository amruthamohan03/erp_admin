...
const MENU_URL = '/masters/truck-statuses';
const MENU_NAME = 'Truck Statuses';
...
export async function seedTruckStatusesMenu(
  ...
  if (existing) {
    ...
    if (!inserted) {
      throw new Error('seedTruckStatusesMenu: insert returned no row');
    }
    menuId = inserted.id;
  }
