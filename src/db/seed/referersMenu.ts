...
const MENU_URL = '/masters/referers';
const MENU_NAME = 'Referrers';
...
export async function seedReferersMenu(
  ...
  if (existing) {
    ...
    if (!inserted) throw new Error('seedReferersMenu: insert returned no row');
    menuId = inserted.id;
  }
