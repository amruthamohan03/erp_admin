...
const MENU_URL = '/masters/document-statuses';
const MENU_NAME = 'Document Statuses';
...
const MENU_DEFAULTS = {
  ...
  menuLevel: 0,
  menuOrder: 44,
  icon: 'ti ti-file-badge',
  text: null,
  badge: null,
  display: 'Y' as const,
};

export async function seedDocumentStatusesMenu(
  ...
  if (existing) {
    ...
    if (!inserted) {
      throw new Error('seedDocumentStatusesMenu: insert returned no row');
    }
    menuId = inserted.id;
  }
