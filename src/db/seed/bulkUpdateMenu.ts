...
const MENU_URL = '/bulk-update';
const MENU_NAME = 'Bulk Update';
...
const MENU_DEFAULTS = {
  ...
  menuId: null,
  menuLevel: 0,
  // 95 — top-level operational tool, sits after the per-module entries
  // and just before /reports (90). Audience is operators/admins running
  // ad-hoc cleanup.
  menuOrder: 95,
  icon: 'ti ti-database-edit',
  text: null,
  badge: null,
  display: 'Y' as const,
};

export async function seedBulkUpdateMenu(
  ...
  if (existing) {
    ...
    if (!inserted) throw new Error('seedBulkUpdateMenu: insert returned no row');
    menuId = inserted.id;
  }
