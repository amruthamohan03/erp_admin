...
// Sidebar entry for /reports + admin permission grant. Sits last in the
// §2 flow (step 7 — "Reporting — surfaces across every stage").

const MENU_URL = '/reports';
const MENU_NAME = 'Reports';
...
const MENU_DEFAULTS = {
  ...
  menuId: null,
  menuLevel: 0,
  menuOrder: 90,  // after every per-module entry (50..70)
...
export async function seedReportsMenu(db: Database | Transaction): Promise<void> {
  ...
  if (existing) {
    ...
    if (!inserted) throw new Error('seedReportsMenu: insert returned no row');
    menuId = inserted.id;
  }
