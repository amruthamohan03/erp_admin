...
const MENU_URL = '/masters/feet-containers';
const MENU_NAME = 'Container Sizes';
...
export async function seedFeetContainersMenu(
  ...
  if (existing) {
    ...
    if (!inserted) {
      throw new Error('seedFeetContainersMenu: insert returned no row');
    }
    menuId = inserted.id;
  }
