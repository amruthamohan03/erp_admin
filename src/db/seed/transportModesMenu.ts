...
const MENU_URL = '/masters/transport-modes';
const MENU_NAME = 'Transport Modes';
...
export async function seedTransportModesMenu(
  ...
  if (existing) {
    ...
    if (!inserted) {
      throw new Error('seedTransportModesMenu: insert returned no row');
    }
    menuId = inserted.id;
  }
