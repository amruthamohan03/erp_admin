...
const MENU_URL = '/masters/items';
const MENU_NAME = 'Items / Services';
...
export async function seedItemsMenu(
  ...
  if (existing) {
    ...
    if (!inserted) throw new Error('seedItemsMenu: insert returned no row');
    menuId = inserted.id;
  }
