...
const MENU_URL = '/masters/quotation-categories';
const MENU_NAME = 'Quotation Categories';
...
const MENU_DEFAULTS = {
  ...
  menuLevel: 0,
  menuOrder: 36,
  icon: 'ti ti-folder-tree',
  text: null,
  badge: null,
  display: 'Y' as const,
};

export async function seedQuotationCategoriesMenu(
  ...
  if (existing) {
    ...
    if (!inserted) {
      throw new Error('seedQuotationCategoriesMenu: insert returned no row');
    }
    menuId = inserted.id;
  }
