...
// Sidebar entry for /mapping/fieldgrants + admin permission grant. Sits
// next to the existing /mapping/roletomenu and /mapping/roletodashboardcard
// pages — all of them are role × ___ administration screens.

const MENU_URL = '/mapping/fieldgrants';
const MENU_NAME = 'Field Grants';
...
const MENU_DEFAULTS = {
  ...
  menuId: null,
  menuLevel: 0,
  // 35 — sits in the role-mapping cluster of sidebar entries.
  menuOrder: 35,
...
export async function seedFieldGrantsMappingMenu(
  ...
  if (existing) {
    ...
    if (!inserted) {
      throw new Error('seedFieldGrantsMappingMenu: insert returned no row');
    }
    menuId = inserted.id;
  }
