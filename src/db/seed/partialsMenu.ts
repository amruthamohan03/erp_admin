...
const MENU_URL = '/masters/partials';
const MENU_NAME = 'Partials';
...
const MENU_DEFAULTS = {
  ...
  menuId: null,
  menuLevel: 0,
  // 38-42 — customs-domain masters cluster, right after items/categories.
  menuOrder: 38,
...
export async function seedPartialsMenu(
  ...
  if (existing) {
    ...
    if (!inserted) throw new Error('seedPartialsMenu: insert returned no row');
    menuId = inserted.id;
  }
