...
// Sidebar entry for /credit-notes + admin permission grant. Mirrors
// invoicesMenu — see licensesMenu for the upsert-by-URL caveat
// (menu_master_t.url isn't unique).

const MENU_URL = '/credit-notes';
const MENU_NAME = 'Credit Notes';
...
const MENU_DEFAULTS = {
  ...
  menuId: null,
  menuLevel: 0,
  menuOrder: 70,  // sits just below Invoices (60) — follows the §2 step order
...
export async function seedCreditNotesMenu(db: Database | Transaction): Promise<void> {
  ...
  if (existing) {
    ...
    if (!inserted) throw new Error('seedCreditNotesMenu: insert returned no row');
    menuId = inserted.id;
  }
