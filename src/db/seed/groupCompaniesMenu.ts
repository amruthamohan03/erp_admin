...
const MENU_URL = '/masters/group-companies';
const MENU_NAME = 'Group Companies';
...
export async function seedGroupCompaniesMenu(
  ...
  if (existing) {
    ...
    if (!inserted)
      throw new Error('seedGroupCompaniesMenu: insert returned no row');
    menuId = inserted.id;
  }
