...
const MENU_URL = '/masters/invoice-banks';
const MENU_NAME = 'Invoice Banks';
...
export async function seedInvoiceBanksMenu(
  ...
  if (existing) {
    ...
    if (!inserted)
      throw new Error('seedInvoiceBanksMenu: insert returned no row');
    menuId = inserted.id;
  }
