...
const MENU_URL = '/masters/clearing-statuses';
const MENU_NAME = 'Clearing Statuses';
...
export async function seedClearingStatusesMenu(
  ...
  if (existing) {
    ...
    if (!inserted) {
      throw new Error('seedClearingStatusesMenu: insert returned no row');
    }
    menuId = inserted.id;
  }
