...
// Sidebar entry for the bank-exchange-rates transactional UI.
// Lives outside /masters because rates are a daily-operations entry
// table, not a master catalogue — the URL matches that.

const MENU_URL = '/bank-exchange-rates';
const MENU_NAME = 'Exchange Rates';
...
export async function seedBankExchangeRatesMenu(
  ...
  if (existing) {
    ...
    if (!inserted)
      throw new Error('seedBankExchangeRatesMenu: insert returned no row');
    menuId = inserted.id;
  }
