...
const MENU_URL = '/masters/transit-points';
const MENU_NAME = 'Transit Points';
...
export async function seedTransitPointsMenu(
  ...
  if (existing) {
    ...
    if (!inserted) {
      throw new Error('seedTransitPointsMenu: insert returned no row');
    }
    menuId = inserted.id;
  }
