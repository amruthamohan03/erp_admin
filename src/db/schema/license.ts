...
// Full customs-license model ported from main's `licenses_t`, kept under the
// restructure export name `licenseT` / SQL table `license_t`.
//
// NOTE on nullability: the source marks license_number, client_id,
// type_of_goods_id, transport_mode_id and currency_id as NOT NULL. The
// transactional-page runtime saves ONE accordion at a time and INSERTs a new
// row from whichever accordion is saved first, so those columns cannot all be
...
export const licenseT = pgTable(
  'license_t',
  {
    ...
    licenseClearedBy: integer('license_cleared_by').references(() => doneByMaster.id),
    ...
    // Status enum: ACTIVE / INACTIVE / ANNULATED / MODIFIED / PROROGATED.
    // (Kept as varchar to mirror the source; a CHECK constraint enforces the
    // set at the migration layer.)
  ...
  (t) => ({
    ...
    // real license/invoice numbers unique.
    licenseNumberUq: uniqueIndex('uq_license_t_license_number')
      .on(t.licenseNumber)
      .where(sql`${t.licenseNumber} IS NOT NULL AND ${t.licenseNumber} <> ''`),
    invoiceNumberUq: uniqueIndex('uq_license_t_invoice_number')
      .on(t.invoiceNumber)
      .where(sql`${t.invoiceNumber} IS NOT NULL AND ${t.invoiceNumber} <> ''`),
    clientIdx: index('idx_license_t_client').on(t.clientId),
    kindIdx: index('idx_license_t_kind').on(t.kindId),
    transportIdx: index('idx_license_t_transport').on(t.transportModeId),
    displayLicenseIdx: index('idx_license_t_display_license').on(t.display, t.licenseNumber),
  }),
);
