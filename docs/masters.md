...
| Table | Schema file | Migration | Purpose |
| ----- | ----------- | --------- | ------- |
| `rule_master_t` | [src/db/schema/rules.ts](../src/db/schema/rules.ts) | [0002_add_master_rules.sql](../drizzle/0002_add_master_rules.sql) | Business rules. `rule_json` body, evaluator picks the format (JSON Logic / CEL / custom DSL). |
...
| `workflow_transition_master_t` | [src/db/schema/workflow.ts](../src/db/schema/workflow.ts) | [0003_add_master_workflow.sql](../drizzle/0003_add_master_workflow.sql) | Transitions between states. Optional `rule_id` gate + `action_json` side-effect blob. |
...
| `form_field_master_t` | [src/db/schema/forms.ts](../src/db/schema/forms.ts) | [0004_add_master_forms.sql](../drizzle/0004_add_master_forms.sql) | Fields of a form definition. `field_type` is a string the renderer maps to a React component. |
| `case_template_master_t` | [src/db/schema/caseTemplate.ts](../src/db/schema/caseTemplate.ts) | [0005_add_master_case_template.sql](../drizzle/0005_add_master_case_template.sql) | The glue: ties a form + a workflow + a target table together. Adding a "case type" is supposed to be a row here. |
