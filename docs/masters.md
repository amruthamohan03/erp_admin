...
| `payment_type_master_t` | [src/db/schema/paymentTypeMaster.ts](../src/db/schema/paymentTypeMaster.ts) | [0032_red_centennial.sql](../drizzle/0032_red_centennial.sql) | The two directions of the DRC "modalité de paiement" declaration codes — seeded as EXPORT (id 1) and IMPORT (id 2). Parent FK for `payment_subtype_master_t`, which holds the codes themselves. Surfaces on the licence form as "Payment Method", scoping the subtype list. Both rows were named after the opposite direction to the codes filed under them until [0047_license_form_fixes.sql](../drizzle/0047_license_form_fixes.sql) corrected them. |
| `payment_subtype_master_t` | [src/db/schema/paymentSubtypeMaster.ts](../src/db/schema/paymentSubtypeMaster.ts) | [0032_red_centennial.sql](../drizzle/0032_red_centennial.sql) | The DRC "modalité de paiement" declaration codes, nested under `payment_type_master_t`: 01–10 are the EXPORTATION modalities, 20–36 the IMPORTATION ones. FK to parent type so pickers can scope by direction — the licence form's Payment Subtype passes the chosen Payment Method as `?payment_type_id=`. |
