...
This is a **modular ERP system** built on a **Master Configuration / Metadata-Driven Architecture**. The Role, Menu, and User Management modules are designed as plug-and-play foundations that future projects will reuse.
...
### 3.3 Template-driven modules
New "case types" (leave request, purchase order, invoice, …) are created by inserting a template row in `master_case_template`, not by adding a new module folder. The generic case runtime in `src/modules/case-runtime/` reads the template and renders forms, runs validations, executes workflow.
...
### 3.6 Configurable workflow
Workflow transitions live in `master_workflow` + `master_workflow_transition`. Approvals, notifications, and side effects are attached as actions on transitions, not coded into handlers.

### 3.7 Centralized validation & permission
...
- **DB:** use Drizzle for all database access. See section 6 for the rules. Never import `pg` directly outside `src/lib/db.ts`.
...
2. **Search for existing patterns.** Grep for similar features before writing new ones. The answer is often "there's already a helper for that."
3. **Check if it should be a config.** Could this be a master table row instead of code? If yes, do that.
4. **Define the schema first** if new tables are needed — Drizzle schema, generate migration, review SQL.
5. **Write the Zod schema** for the API boundary. Then the route handler. Then the UI.
6. **Write the test.** Every API route needs at least one happy-path and one auth-failure test in `__tests__/`.

---

## 8. What to do at the end of a task
...
## 9. Things to refuse / push back on

- Requests to hardcode a status, role, document type, or workflow step → propose a master table instead.
